...
| 状态 | 任务 | 说明 |
|------|------|------|
| ✅ | 完善 UI 功能 | RSA 操作界面已集成 |
| ✅ | 实时预览 | 用户输入时自动加密显示（可开关） |
| ⏳ | 实现文件交互 | 支持密钥和密文文件导入导出 |
| ⏳ | 错误处理 | 添加输入验证和异常处理 |
| ⏳ | 用户体验 | 状态提示、进度显示等 |
...
**功能说明：**
- 用户在左侧 Plaintext 输入文本时，右侧自动显示加密结果
- 通过 Switch 开关控制是否启用实时预览
- 采用 500ms 节流策略，避免频繁加密计算
- 状态栏显示预览状态（成功/错误提示）

**技术实现：**
- 使用 `Timer` 轮询检测文本变化（500ms 间隔）
- 只在文本真正变化时触发加密
- 加密失败时显示具体错误信息
